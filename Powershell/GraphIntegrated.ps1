...
try {
    ...
    $builder = $builder.WithWindowsBroker($true)
    $app = $builder.Build()

    Write-Host "`nAttempting to acquire token using Windows Integrated Authentication..."
    try {
        # Try Windows Integrated Auth first
        $result = $app.AcquireTokenByIntegratedWindowsAuth($scopes).ExecuteAsync().GetAwaiter().GetResult()
    } catch {
        Write-Error "Windows Integrated Authentication failed: $_"
        Write-Host "This might be because:"
        Write-Host "1. Your Windows account is not linked to Azure AD"
        Write-Host "2. Your organization doesn't support Windows Integrated Authentication"
        Write-Host "3. Multi-factor authentication is required"
        exit 1
    }
